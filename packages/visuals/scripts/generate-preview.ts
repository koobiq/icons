...
async function generateRow(baseName: string) {
    const retinaName = baseName.includes('@2x') ? '(Retina)' : '';
    const pngSrc = `${URL_PREFIX}/${baseName}.png`;
    const lossyWebp = `${URL_PREFIX}/${baseName}.webp`;

    // Get file sizes
    const pngSize = await getFileSize(path.join(DEST_DIR, `${baseName}.png`));
    const lossyWebpSize = await getFileSize(path.join(DEST_DIR, `${baseName}.webp`));
...
async function generateHtml() {
    try {
        ...
        for (const file of pngFiles) {
            ...
            const row = await generateRow(baseName);
